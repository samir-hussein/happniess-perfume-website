...
class PlaceOrderHandler
{
	...
	public function __invoke(array $data)
	{
		...
		$shipping = $this->calculateShipping($data['city']);
	...
	private function calculateShipping($governorate)
	{
		$remoteAreas = [
			'matruh',
			'new_valley',
		...
		if (in_array($governorate, $remoteAreas)) {
			return ceil(ShippingMethod::where('name_en', 'Remote Areas')->first()->cost);
		}

		if ($governorate === 'cairo' || $governorate === 'giza') {
			return ceil(ShippingMethod::where('name_en', 'Cairo & Giza')->first()->cost);
		}

		return ceil(ShippingMethod::where('name_en', 'Other Governorates')->first()->cost);
	}
